...
//                                        "api/admin/getUsers", "api/admin/adminIndex","api/admin/adminPage" ,"api/admin/addUser")
                                        .hasAuthority(UserDao.Role.ADMIN.name())
                                .requestMatchers("/api/user/**")
                                    .hasAuthority(UserDao.Role.USER.name())
                                    ...
                                    .successHandler((request, response, authentication) -> {
                                            for (GrantedAuthority authority : authentication.getAuthorities()) {
                                                if (authority.getAuthority().equals(UserDao.Role.USER.toString())) {
                                                    response.sendRedirect("/api/user/userIndex"); // Redirect to user page for ROLE_USER
                                                    return;
                                                } else if (authority.getAuthority().equals(UserDao.Role.ADMIN.toString())) {
                                                    response.sendRedirect("/api/admin/adminIndex"); // Redirect to admin page for ROLE_ADMIN
                                                    return;
                                                }
                                            }
                                        })
