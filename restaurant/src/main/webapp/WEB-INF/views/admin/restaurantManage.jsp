                    ...
                    <c:when test="${content.size() > 0}">
                        <c:forEach var="complaint" items="${content}" varStatus="ind">
                            <tr align="center">
                                ...
                                <td>${complaint.restaurantName}</td>
                                <td>${complaint.contactNumber}</td>
                                <td>${complaint.toString()}</td>
                                <td>
                                    <a href="editRestaurant/${complaint.userId}" class="btn btn-outline-info btn-sm">Edit</a>
                                    <a onclick="deleteWithId('deleteRestaurant', ${complaint.userId}, 'manageRestaurant')" class="btn btn-outline-danger btn-sm">Delete</a>
                                </td>
                            </tr>
                        </c:forEach>
                    </c:when>
            ...
            <div class="page-footer">
                ...
                <ul class="pagination mb-0">
                    ...
                    <li class="page-item ${currentPage == 1 ? 'disabled' : ''}">
                        <a onclick="populateWithPage('manageRestaurant', '${currentPage - 1}')" class="page-link" aria-label="Previous">
                            <span aria-hidden="true">&laquo;</span>
                        </a>
                    </li>
                    <c:forEach begin="0" end="${totalPages-1}" var="page">
                        <li class="page-item ${page + 1 == currentPage ? 'active' : ''}">
                            <a onclick="populateWithPage('manageRestaurant', '${page+1}')" class="page-link">${page+1}</a>
                        </li>
                    </c:forEach>
                    <li class="page-item ${currentPage == totalPages ? 'disabled' : ''}">
                        <a onclick="populateWithPage('manageRestaurant', '${currentPage + 1}')" class="page-link" aria-label="Next">
